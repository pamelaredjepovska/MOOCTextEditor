...
public class AutoCompleteMatchCase implements Dictionary, AutoComplete {
	...
	public boolean addWord(String word) {
		...
		// If it doesn't end a node, set the isWord var to true
		if (!curr.endsWord()) {
	...
	public boolean isWord(String s) {
		...
		curr = searchTrieNode(s);

		if (curr != null && curr.endsWord()) {
			return true;
		}

		if (Character.isUpperCase(s.charAt(0))) // If the first letter is upper case (HELLO, Hello, HeLLO..)
		{
			s1 = Character.toLowerCase(s.charAt(0)) + s.substring(1); // With first letter lower
			curr = searchTrieNode(s1);

			if (curr != null && curr.endsWord()) {
				return true;
			}
		} else if (s.equals(s.toUpperCase())) // All caps (HELLO)
		{
			s2 = s1.toLowerCase(); // All lower case (hello)
			curr = searchTrieNode(s2);

			if (curr != null && curr.endsWord()) {
				return true;
			}

			s3 = Character.toUpperCase(s2.charAt(0)) + s2.substring(1); // With first letter upper (Hello)
			curr = searchTrieNode(s3);
		...
		/*
		 * if (curr != null && curr.endsWord()) { return true; }
		 */
