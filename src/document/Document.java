...
public abstract class Document {
	...
	private String text;
	public static final float FIRST_CONSTANT = 206.835f;
	public static final float SECOND_CONSTANT = 1.015f;
	public static final float THIRD_CONSTANT = 84.6f;
	...
	// This is a method that does not always work correctly
	// TODO: improve this method
	/*
	 * protected int countSyllables(String word) { // TODO: Implement this method so
	 * that you can call it from the // getNumSyllables method in BasicDocument
	 * (module 2) and // EfficientDocument (module 3).
	 * 
	 * // Split the word by anything that's not a vowel, // convert it to list, and
	 * remove empty strings and null elements
	 * 
	 * List<String> vowelsList = new
	 * ArrayList<String>(Arrays.asList(word.split("[^aeiouyAEIOUY]+")));
	 * vowelsList.removeAll(Arrays.asList("", null));
	 * 
	 * // Get the last syllable String lastVowelGrp =
	 * vowelsList.get(vowelsList.size()-1);
	 * 
	 * //System.out.println(vowelsList + " | " + vowelsList.size() + " | last: " +
	 * vowelsList.get(vowelsList.size()-1));
	 * 
	 * // If the word ends with 'e' and that is the last syllable as well, // check
	 * if that is the only syllable or not
	 * 
	 * if (word.endsWith("e") && lastVowelGrp.equals("e")) { // If 'e' is the only
	 * syllable and at the end of the word, return 1 if (vowelsList.size() == 1) {
	 * return vowelsList.size(); } else // Return the number of syllables - 1 (minus
	 * that 'e' at the end) { return vowelsList.size() - 1; } } else // If it
	 * doesn't end with 'e', return the number of syllables { return
	 * vowelsList.size(); } }
	 */

	// This method works correctly and was provided by the MOOC team
	...
	public double getFleschScore() {
		...
		double fleshScore = FIRST_CONSTANT - (SECOND_CONSTANT * (numWords / numSentences))
				- (THIRD_CONSTANT * (numSyllables / numWords));
