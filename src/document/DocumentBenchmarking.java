...
public class DocumentBenchmarking {
	...
	public static void main(String[] args) {
		...
		for (int numToCheck = start; numToCheck < (numSteps * increment) + start; numToCheck += increment) {
			...
			/*
			 * 3. Time a loop that runs trials times (trials is the variable above) that: a.
			 * Creates a BasicDocument b. Calls fleshScore on this document
			 */
			long basicStartTime = System.nanoTime();
			for (int i = 0; i < trials; i++) {
				BasicDocument basicDoc = new BasicDocument(text);
				double basicFleschScore = basicDoc.getFleschScore();
				// System.out.print(fleschScore);
				// System.out.print('\n');
			}
			long basicEndTime = System.nanoTime();

			/*
			 * 4. Print out the time it took to complete the loop in step 3 (on the same
			 * line as the first print statement) followed by a tab (\t)
			 */
			double basicEstTime = (basicEndTime - basicStartTime) / 1000000000.0 / trials;
			System.out.print(basicEstTime + "\t\t");

			/*
			 * 5. Time a loop that runs trials times (trials is the variable above) that: a.
			 * Creates an EfficientDocument b. Calls fleshScore on this document
			 */
			long efficientStartTime = System.nanoTime();
			for (int i = 0; i < trials; i++) {
				EfficientDocument efficientDoc = new EfficientDocument(text);
				double efficientFleschScore = efficientDoc.getFleschScore();
				// System.out.print(fleschScore);
				// System.out.print('\n');
			}
			...
			System.out.println(efficientEstTime + "\t");
